            ...
            <div>
                ...
                <div style="display: none; padding-bottom: 10px;" id="menuSearch">
                    <div style="margin:0 auto" id="typeWrite">
                        ...
                        <v-form @submit.stop ='stop()' id="inputSearch" style="margin-top: 15px; display:none; margin-left: 23px;">
                            <div>
                                <input id="searchValue" autofocus style="outline-style: none;max-width: 700px;width:70%; font-size: 15px;" type="text" name="search" placeholder="Tìm kiếm tên dự án…">

                            </div>
                        </v-form>
                        <v-btn color="red" @click="SearchKey()" icon style="float: right; margin-top: -29px; margin-right: 17px;"><v-icon>search</v-icon></v-btn>
                    </div>
                </div>
            </div>
        </div>
...
computed:{
    ...mapGetters(['allCategories', 'allBlogs'])
},
methods:{
    ...mapActions(['getMenuTop','getByCategory', 'searchBlog']),
    searchClick() {
    ...
    SearchKey(){
        ...
        this.searchBlog ({value: value})
        this.$router.push({ name: 'listSearch', params: { value: value } })
    },
    listBlog(menu){
